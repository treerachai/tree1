...
def loggedIn(func):
    def checkLogin(*args, **kwargs):
        if args[0].isLogin:
            ...
            args[0].callback.other('SELFBPT PY.3BY.SAI	')
    return checkLogin
...
class Object(object):
    ...
    def __init__(self):
        if self.isLogin == True:
            self.log("[ %s ] : โทเคนของระบบไลน์ที่ล็อคอิน" % self.authToken)
            self.log("[ %s ] : Mid ของระบบไลน์ที่ล้อคอิน" % self.profile.mid)
            self.log("[ %s ] : ชื่อของไลน์ที่ทำการล็อคอิน" % self.profile.displayName)
